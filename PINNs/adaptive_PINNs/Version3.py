...
def window_function(x,xL,xR,delta=0.02):
    ...
    return 0.5*(
        torch.tanh((x-xL)/delta)
        -
        torch.tanh((x-xR)/delta)
    )
...
def save_solution(
    ...
    plt.savefig(
        f"{output_folder}/epoch_{epoch:05d}.png",
        dpi=150,
        bbox_inches='tight'
    )
